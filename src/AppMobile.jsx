...
if (!document.getElementById('mobile-font')) {
  ...
  s.href = 'https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700;800&display=swap';
...
function Splash({ onDone }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, ...FONT }}>
      <div style={{ position: 'absolute', top: 0, left: 0, width: '50%', height: '100%', background: '#fff', transform: phase ? 'translateX(-100%)' : 'none', transition: 'transform 0.65s cubic-bezier(0.77,0,0.175,1)' }} />
      <div style={{ position: 'absolute', top: 0, right: 0, width: '50%', height: '100%', background: '#fff', transform: phase ? 'translateX(100%)' : 'none', transition: 'transform 0.65s cubic-bezier(0.77,0,0.175,1)' }} />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, opacity: phase ? 0 : 1, transition: 'opacity 0.3s' }}>
        <div style={{ width: 84, height: 84, borderRadius: 28, background: '#1B4F8A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 40, boxShadow: '0 8px 32px rgba(27,79,138,0.35)' }}>🌊</div>
        <div style={{ textAlign: 'center' }}>
          <p style={{ margin: 0, fontSize: 28, fontWeight: 800, color: '#0f172a', letterSpacing: -0.5 }}>Co na Mazurach?</p>
          <p style={{ margin: '6px 0 0', fontSize: 13, color: '#64748b', fontWeight: 500 }}>Wszystko co potrzebujesz w jednym miejscu</p>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ overflowX: 'auto', display: 'flex', gap: 8, padding: '0 16px 2px', scrollbarWidth: 'none' }}>
      {CATS.map(c => (
        <button key={c.id} onClick={() => onChange(c.id)} style={{
          ...
          background: active === c.id ? '#1B4F8A' : '#F1F5F9',
...
function Card({ listing, onClick, favs, toggleFav }) {
  const c = cat(listing.category);
  const isFav = favs?.has(String(listing.id));
  return (
    <div onClick={() => onClick(listing)} style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', boxShadow: '0 2px 16px rgba(0,0,0,0.08)', cursor: 'pointer', position: 'relative' }}>
      ...
      <div style={{ padding: '12px 14px 14px' }}>
        <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#0f172a', lineHeight: 1.3 }}>{listing.title}</p>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 5 }}>
          <span style={{ fontSize: 12, color: '#64748b', fontWeight: 500 }}>📍 {listing.city}</span>
          {listing.rating ? <span style={{ fontSize: 12, color: '#F4A825', fontWeight: 700 }}>⭐ {listing.rating}</span> : null}
        </div>
        {listing.date && <p style={{ margin: '4px 0 0', fontSize: 12, color: '#1B4F8A', fontWeight: 600 }}>📅 {new Date(listing.date).toLocaleDateString('pl-PL', { day: 'numeric', month: 'long' })}{listing.time ? ` · ${listing.time}` : ''}</p>}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
          <p style={{ margin: 0, fontSize: 14, fontWeight: 800, color: '#1B4F8A' }}>{listing.priceLabel || (listing.price === 0 ? 'Bezpłatne' : `${listing.price} zł`)}</p>
          <div style={{ display: 'flex', gap: 4 }}>
            {(listing.tags || []).slice(0, 2).map(t => (
              <span key={t} style={{ fontSize: 10, color: '#64748b', background: '#F1F5F9', padding: '2px 7px', borderRadius: 999, fontWeight: 600 }}>#{t}</span>
            ))}
          </div>
        </div>
      </div>
...
// ─── Detail screen ────────────────────────────────────────────────────────────
function DetailScreen({ listing, onBack, favs, toggleFav }) {
  const c = cat(listing.category);
  const isFav = favs?.has(String(listing.id));
  ...
  return (
    <div style={{ ...FONT, height: '100%', overflowY: 'auto', background: '#F7F4EE' }}>
      {/* Hero */}
      ...
      <div style={{ padding: 20, paddingBottom: 100 }}>
        ...
        <h1 style={{ margin: '0 0 8px', fontSize: 22, fontWeight: 800, color: '#0f172a', lineHeight: 1.2 }}>{listing.title}</h1>

        {/* Meta */}
        <div style={{ background: '#fff', borderRadius: 16, padding: 14, marginBottom: 14 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <p style={{ margin: 0, fontSize: 13, color: '#475569', fontWeight: 500 }}>📍 {listing.address}, {listing.city}</p>
            {listing.date && <p style={{ margin: 0, fontSize: 13, color: '#475569' }}>📅 {new Date(listing.date).toLocaleDateString('pl-PL', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>}
            {listing.time && <p style={{ margin: 0, fontSize: 13, color: '#475569' }}>🕐 {listing.time}</p>}
          </div>
        </div>
        ...
        {listing.description && (
          <div style={{ background: '#fff', borderRadius: 16, padding: 16, marginBottom: 14 }}>
            <p style={{ margin: '0 0 8px', fontWeight: 700, fontSize: 15, color: '#0f172a' }}>Opis</p>
            <div style={{ fontSize: 14, color: '#334155', lineHeight: 1.7 }} dangerouslySetInnerHTML={{ __html: listing.description }} />
          </div>
        )}
        ...
        {listing.features?.length > 0 && (
          <div style={{ background: '#fff', borderRadius: 16, padding: 16, marginBottom: 14 }}>
            <p style={{ margin: '0 0 10px', fontWeight: 700, fontSize: 15, color: '#0f172a' }}>Udogodnienia</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
              {listing.features.map((f, i) => (
                <p key={i} style={{ margin: 0, fontSize: 13, color: '#475569' }}>✓ {f}</p>
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              <p style={{ margin: 0, fontSize: 12, color: '#64748b' }}>Cena</p>
            ...
            {listing.rating && (
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ margin: 0, fontSize: 11, color: '#64748b' }}>ocena</p>
              </div>
            )}
          </div>
          {listing.phone && <p style={{ margin: '0 0 12px', fontSize: 14, color: '#475569', fontWeight: 500 }}>📞 {listing.phone}</p>}
          ...
          <a href={mailto} style={{
            display: 'block', width: '100%', padding: '15px', borderRadius: 16,
            background: '#1B4F8A', color: '#fff', border: 'none',
            fontSize: 16, fontWeight: 700, textAlign: 'center', textDecoration: 'none',
          }}>
...
// ─── Discover screen ──────────────────────────────────────────────────────────
function DiscoverScreen({ listings, onSelect, favs, toggleFav }) {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: '#F7F4EE' }}>
      {/* Header */}
      <div style={{ padding: '16px 16px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <p style={{ margin: 0, fontSize: 13, color: '#64748b', fontWeight: 500 }}>Cześć! 👋</p>
          <p style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#0f172a' }}>Co na Mazurach?</p>
        </div>
        <button onClick={() => setShowSearch(s => !s)} style={{ width: 42, height: 42, borderRadius: 999, background: '#1B4F8A', border: 'none', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🔍</button>
      </div>
      ...
      {showSearch && (
        <div style={{ padding: '0 16px 12px' }}>
          <input
            ...
            style={{ width: '100%', padding: '12px 16px', borderRadius: 14, border: '2px solid #1B4F8A', fontSize: 14, ...FONT, boxSizing: 'border-box', outline: 'none' }}
          />
        </div>
      )}
      ...
      <div style={{ padding: '0 16px 80px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {filtered.map(l => <Card key={l.id} listing={l} onClick={onSelect} favs={favs} toggleFav={toggleFav} />)}
      </div>
    </div>
  );
}
...
// ─── Map screen ───────────────────────────────────────────────────────────────
function MapScreen({ listings, onSelect, favs, toggleFav }) {
  const withCoords = listings.filter(l => l.lat && l.lng);
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '16px 16px 8px' }}>
        <p style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#0f172a' }}>Mapa Mazur</p>
        <p style={{ margin: '2px 0 0', fontSize: 13, color: '#64748b' }}>{withCoords.length} miejsc na mapie</p>
      </div>
...
// ─── Calendar screen ──────────────────────────────────────────────────────────
function CalendarScreen({ listings, onSelect, favs, toggleFav }) {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: '#F7F4EE' }}>
      ...
      <div style={{ margin: 16, borderRadius: 20, overflow: 'hidden', boxShadow: '0 2px 16px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ background: '#fff', padding: '8px 12px 12px' }}>
      ...
      <div style={{ padding: '0 16px 80px' }}>
        {selDay ? (
          <>
            <p style={{ margin: '0 0 12px', fontSize: 16, fontWeight: 700, color: '#0f172a' }}>
              {selDay} {MONTHS[month]} {year} · {dayListings.length} {dayListings.length === 1 ? 'wydarzenie' : 'wydarzeń'}
            </p>
            {dayListings.length === 0
              ? <div style={{ background: '#fff', borderRadius: 16, padding: 20, textAlign: 'center', color: '#94a3b8' }}><p>Brak wydarzeń tego dnia</p></div>
              : dayListings.map(l => <div key={l.id} style={{ marginBottom: 12 }}><Card listing={l} onClick={onSelect} favs={favs} toggleFav={toggleFav} /></div>)
            }
          </>
        ) : (
          <>
            ...
            {listings.filter(l => l.date && new Date(l.date) >= today).sort((a, b) => new Date(a.date) - new Date(b.date)).slice(0, 5).map(l => (
              <div key={l.id} style={{ marginBottom: 12 }}><Card listing={l} onClick={onSelect} favs={favs} toggleFav={toggleFav} /></div>
            ))}
            {listings.filter(l => l.date && new Date(l.date) >= today).length === 0 && (
              <div style={{ background: '#fff', borderRadius: 16, padding: 20, textAlign: 'center', color: '#94a3b8' }}>
...
function AddListingScreen() {
  ...
  const inputStyle = { width: '100%', padding: '13px 16px', borderRadius: 14, border: '1.5px solid #E2E8F0', fontSize: 14, ...FONT, boxSizing: 'border-box', outline: 'none', background: '#fff' };
      ...
      <div style={{ ...FONT, height: '100%', overflowY: 'auto', background: '#F7F4EE' }}>
        <div style={{ padding: 20, paddingBottom: 80 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #1B4F8A, #2563EB)', borderRadius: 24, padding: 28, marginBottom: 20, textAlign: 'center' }}>
            ...
            <p style={{ margin: '8px 0 0', fontSize: 13, color: 'rgba(255,255,255,0.8)', lineHeight: 1.5 }}>
              Formularz wypełniony. Teraz wyślij nam wiadomość — otworzy się poczta z gotową wiadomością. Wystarczy kliknąć Wyślij!
            </p>
          </div>

          {/* Podsumowanie */}
          <div style={{ background: '#fff', borderRadius: 20, padding: 18, marginBottom: 16 }}>
            <p style={{ margin: '0 0 12px', fontWeight: 700, fontSize: 14, color: '#0f172a' }}>📋 Podsumowanie zgłoszenia</p>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                <p style={{ margin: 0, fontSize: 12, color: '#94a3b8', width: 68, flexShrink: 0, fontWeight: 600 }}>{row.label}</p>
                <p style={{ margin: 0, fontSize: 13, color: '#334155', fontWeight: 500, wordBreak: 'break-all' }}>{row.val}</p>
              </div>
            ))}
          </div>

          <p style={{ margin: '-8px 0 16px', fontSize: 12, color: '#94a3b8', paddingLeft: 4 }}>
            📝 Opis ogłoszenia zostanie dołączony do wiadomości email.
          </p>

          {/* Jak to działa */}
          <div style={{ background: '#FFF7ED', borderRadius: 16, padding: 16, marginBottom: 16, display: 'flex', gap: 12 }}>
            <span style={{ fontSize: 22, flexShrink: 0 }}>ℹ️</span>
            <div>
              <p style={{ margin: '0 0 4px', fontSize: 13, fontWeight: 700, color: '#92400E' }}>Jak to działa?</p>
              <p style={{ margin: 0, fontSize: 12, color: '#92400E', lineHeight: 1.5 }}>
                Kliknięcie przycisku otworzy pocztę z gotową wiadomością. Sprawdź treść i kliknij Wyślij. Ogłoszenie pojawi się na portalu po weryfikacji.
              </p>
            </div>
          </div>

          {/* CTA */}
          <a href={mailto} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '17px', borderRadius: 18, background: '#1B4F8A', color: '#fff', textAlign: 'center', fontWeight: 800, fontSize: 17, textDecoration: 'none', marginBottom: 12, boxShadow: '0 4px 16px rgba(27,79,138,0.35)' }}>
            <span style={{ fontSize: 22 }}>📧</span> Otwórz pocztę i wyślij
          </a>
          <p style={{ textAlign: 'center', fontSize: 11, color: '#94a3b8', margin: '0 0 16px' }}>
            Wyślij na: {CONTACT_EMAIL}
          </p>

          <button onClick={() => { setDone(false); setStep(0); setForm(EMPTY_FORM); }} style={{ width: '100%', padding: '14px', borderRadius: 16, border: '2px solid #E2E8F0', background: '#fff', color: '#64748b', fontWeight: 700, fontSize: 14, cursor: 'pointer', ...FONT }}>
  ...
  return (
    <div style={{ ...FONT, height: '100%', overflowY: 'auto', background: '#F7F4EE' }}>
      <div style={{ padding: '16px 16px 0' }}>
        <p style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#0f172a' }}>Dodaj ogłoszenie</p>
        <p style={{ margin: '2px 0 16px', fontSize: 13, color: '#64748b' }}>Krok {step + 1} z 3</p>
        {/* Progress */}
        <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>
          {[0,1,2].map(i => <div key={i} style={{ flex: 1, height: 4, borderRadius: 999, background: i <= step ? '#1B4F8A' : '#E2E8F0', transition: 'background 0.3s' }} />)}
        </div>
      </div>
      ...
      <div style={{ padding: '0 16px 80px' }}>
        {step === 0 && (
          <div style={{ background: '#fff', borderRadius: 20, padding: 20 }}>
            <p style={{ margin: '0 0 12px', fontWeight: 700, fontSize: 15, color: '#0f172a' }}>Kategoria *</p>
            ...
            <p style={{ margin: '0 0 6px', fontWeight: 600, fontSize: 13, color: '#374151' }}>Tytuł ogłoszenia *</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div>
                <p style={{ margin: '0 0 6px', fontWeight: 600, fontSize: 13, color: '#374151' }}>Miasto *</p>
                <input value={form.city} onChange={e => f('city', e.target.value)} placeholder="np. Giżycko" style={inputStyle} />
              </div>
              <div>
                <p style={{ margin: '0 0 6px', fontWeight: 600, fontSize: 13, color: '#374151' }}>Adres</p>
        ...
        {step === 1 && (
          <div style={{ background: '#fff', borderRadius: 20, padding: 20 }}>
            <p style={{ margin: '0 0 6px', fontWeight: 600, fontSize: 13, color: '#374151' }}>Opis *</p>
            <textarea value={form.description} onChange={e => f('description', e.target.value)} placeholder="Opisz swoje ogłoszenie szczegółowo..." rows={5} style={{ ...inputStyle, resize: 'none', marginBottom: 14 }} />
            <p style={{ margin: '0 0 6px', fontWeight: 600, fontSize: 13, color: '#374151' }}>Cena</p>
            ...
            <p style={{ margin: '0 0 6px', fontWeight: 600, fontSize: 13, color: '#374151' }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 400, color: '#94a3b8', marginLeft: 6 }}>(opcjonalne)</span>
            </p>
            <input
              value={form.website}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setStep(0)} style={{ flex: 1, padding: '14px', borderRadius: 16, border: '2px solid #E2E8F0', background: '#fff', color: '#64748b', fontWeight: 700, fontSize: 14, cursor: 'pointer', ...FONT }}>← Wróć</button>
              <button onClick={() => setStep(2)} disabled={!form.description} style={{ flex: 2, padding: '14px', borderRadius: 16, background: !form.description ? '#E2E8F0' : '#1B4F8A', color: !form.description ? '#94a3b8' : '#fff', border: 'none', fontWeight: 700, fontSize: 15, cursor: 'pointer', ...FONT }}>Dalej →</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ background: '#fff', borderRadius: 20, padding: 20 }}>
            <p style={{ margin: '0 0 6px', fontWeight: 600, fontSize: 13, color: '#374151' }}>Twoje imię i nazwisko *</p>
            <input value={form.name} onChange={e => f('name', e.target.value)} placeholder="Jan Kowalski" style={{ ...inputStyle, marginBottom: 14 }} />
            <p style={{ margin: '0 0 6px', fontWeight: 600, fontSize: 13, color: '#374151' }}>Email</p>
            ...
            <p style={{ margin: '0 0 6px', fontWeight: 600, fontSize: 13, color: '#374151' }}>Telefon</p>
            <input value={form.phone} onChange={e => f('phone', e.target.value)} placeholder="+48 123 456 789" style={{ ...inputStyle, marginBottom: 20 }} />
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setStep(1)} style={{ flex: 1, padding: '14px', borderRadius: 16, border: '2px solid #E2E8F0', background: '#fff', color: '#64748b', fontWeight: 700, fontSize: 14, cursor: 'pointer', ...FONT }}>← Wróć</button>
              <button onClick={() => setDone(true)} disabled={!form.name} style={{ flex: 2, padding: '14px', borderRadius: 16, background: !form.name ? '#E2E8F0' : '#2E9E6E', color: !form.name ? '#94a3b8' : '#fff', border: 'none', fontWeight: 700, fontSize: 15, cursor: 'pointer', ...FONT }}>✓ Generuj wiadomość</button>
            </div>
          </div>
        )}
      </div>
...
// ─── Favorites screen ─────────────────────────────────────────────────────────
function FavoritesScreen({ listings, favs, onSelect, toggleFav, onBack }) {
  const favListings = listings.filter(l => favs.has(String(l.id)));
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: '#F7F4EE' }}>
      <div style={{ padding: '16px 16px 8px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={onBack} style={{ width: 38, height: 38, borderRadius: 999, background: '#fff', border: 'none', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>←</button>
        <div>
          <p style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#0f172a' }}>❤️ Ulubione</p>
          <p style={{ margin: 0, fontSize: 13, color: '#64748b' }}>{favListings.length} {favListings.length === 1 ? 'oferta' : 'ofert'}</p>
        </div>
      </div>
      ...
      <div style={{ padding: '8px 16px 80px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        {favListings.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#94a3b8' }}>
            ...
            <p style={{ fontSize: 17, fontWeight: 700, margin: '0 0 6px', color: '#64748b' }}>Brak ulubionych</p>
            <p style={{ fontSize: 13, margin: 0 }}>Dotknij serduszka 🤍 na dowolnym kafelku, żeby go tu zapisać</p>
          </div>
        ) : (
          favListings.map(l => <Card key={l.id} listing={l} onClick={onSelect} favs={favs} toggleFav={toggleFav} />)
        )}
      </div>
    </div>
  );
...
// ─── Profile screen ────────────────────────────────────────────────────────────
function ProfileScreen({ favs, onShowFavs }) {
    ...
    <div style={{ height: '100%', overflowY: 'auto', background: '#F7F4EE' }}>
      <div style={{ padding: 20 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', boxShadow: '0 2px 16px rgba(0,0,0,0.07)' }}>
          {items.map((item, i) => (
            <div key={i} onClick={item.action} style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, borderBottom: i < items.length - 1 ? '1px solid #F1F5F9' : 'none', cursor: item.action ? 'pointer' : 'default', background: item.highlight ? '#FFF5F5' : 'transparent' }}>
              <span style={{ fontSize: 24, width: 40, height: 40, borderRadius: 12, background: item.highlight ? '#FEE2E2' : '#F7F4EE', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{item.icon}</span>
              <div style={{ flex: 1 }}>
                <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: item.highlight ? '#991B1B' : '#0f172a' }}>{item.label}</p>
                <p style={{ margin: '2px 0 0', fontSize: 12, color: '#94a3b8' }}>{item.sub}</p>
              </div>
              {item.action && <span style={{ color: '#cbd5e1', fontSize: 18 }}>›</span>}
            </div>
          ))}
        </div>
        <p style={{ textAlign: 'center', marginTop: 24, fontSize: 12, color: '#94a3b8' }}>Co na Mazurach? v1.0.0</p>
      </div>
    </div>
  );
}
...
function BottomNav({ active, onChange }) {
  return (
    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #F1F5F9', display: 'flex', paddingBottom: 'env(safe-area-inset-bottom)', boxShadow: '0 -4px 20px rgba(0,0,0,0.06)', zIndex: 50 }}>
      {NAV.map(n => (
        <button key={n.id} onClick={() => onChange(n.id)} style={{
          ...
          {n.id === 'dodaj' ? (
            <div style={{ width: 44, height: 44, borderRadius: 999, background: '#1B4F8A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, boxShadow: '0 4px 12px rgba(27,79,138,0.4)', marginTop: -16 }}>➕</div>
          ) : (
            <span style={{ fontSize: 22, lineHeight: 1 }}>{n.icon}</span>
          )}
          <span style={{ fontSize: 10, fontWeight: active === n.id ? 700 : 500, color: active === n.id ? '#1B4F8A' : '#94a3b8', ...FONT }}>{n.label}</span>
...
export default function AppMobile() {
  ...
  if (detail) {
    return (
      <div style={{ ...FONT, position: 'fixed', inset: 0, background: '#F7F4EE', overflow: 'hidden' }}>
        <DetailScreen listing={detail} onBack={() => setDetail(null)} favs={favs} toggleFav={toggleFav} />
      </div>
    );
  }
    ...
    return (
      <div style={{ ...FONT, position: 'fixed', inset: 0, background: '#F7F4EE', overflow: 'hidden' }}>
        <FavoritesScreen
          ...
          favs={favs}
          toggleFav={toggleFav}
          onSelect={l => { setShowFavs(false); setDetail(l); }}
          onBack={() => setShowFavs(false)}
        />
      </div>
    );
  }
    ...
    <div style={{ ...FONT, position: 'fixed', inset: 0, background: '#F7F4EE', overflow: 'hidden' }}>
      ...
      {loading && !splash && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12 }}>
          ...
          <p style={{ margin: 0, color: '#64748b', fontWeight: 600 }}>Ładowanie ogłoszeń...</p>
        </div>
      )}
      ...
      {!loading && (
        <>
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 65, overflowY: 'hidden' }}>
            {tab === 'odkryj'    && <DiscoverScreen  listings={listings} onSelect={setDetail} favs={favs} toggleFav={toggleFav} />}
            {tab === 'mapa'      && <MapScreen        listings={listings} onSelect={setDetail} favs={favs} toggleFav={toggleFav} />}
            {tab === 'dodaj'     && <AddListingScreen />}
            {tab === 'kalendarz' && <CalendarScreen   listings={listings} onSelect={setDetail} favs={favs} toggleFav={toggleFav} />}
            {tab === 'profil'    && <ProfileScreen favs={favs} onShowFavs={() => setShowFavs(true)} />}
          </div>
